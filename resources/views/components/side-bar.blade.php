...
<aside id="default-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidenav">
    <div class="overflow-y-auto py-5 px-3 h-full bg-white border-r border-gray-200">
        ...
        <div class="flex flex-col items-center pb-4 mb-4 border-b border-gray-200">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="mt-3 w-full flex items-center justify-center px-4 py-2 text-base font-medium text-gray-900 rounded-lg bg-gray-200 hover:bg-gray-300 logout-btn">
        ...
        <ul class="space-y-2">
            ...
            <li>              
                <a href="/expenses" class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg hover:bg-gray-100 group">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="flex-shrink-0 w-6 h-6 text-gray-400 transition duration-75 group-hover:text-gray-900">
    ...
    <div class="hidden absolute bottom-0 left-0 justify-center p-4 space-x-4 w-full lg:flex bg-white z-20 border-r border-gray-200">
        <a href="/profile" data-tooltip-target="tooltip-settings" class="inline-flex justify-center p-2 text-gray-500 rounded cursor-pointer hover:text-gray-900 hover:bg-gray-100">
